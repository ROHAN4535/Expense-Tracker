  ...
  const deleteHandler = async (item) => {
    ...
    try {
      const res = await axios.get(
        `https://expense-tracker-f082a-default-rtdb.firebaseio.com/${email}/expenses.json`
      );
      const data = res.data;
      const Id = Object.keys(data).find((eleId) => data[eleId].id === item.id);
      try {
        const res = await axios.delete(
          `https://expense-tracker-f082a-default-rtdb.firebaseio.com/${email}/expenses/${Id}.json`
        );
      } catch (err) {
        alert(err);
      }
  ...
  const clickAccountPremiumHandler = async () => {
    ...
    try {
      const res = axios.post(
        `https://expense-tracker-f082a-default-rtdb.firebaseio.com/${email}/userDetail.json`,
        { isPremium: true }
      );
    } catch (err) {
      alert(err);
